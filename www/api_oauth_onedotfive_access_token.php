<?php
	...
	$grant_token = api_oauth_onedotfive_grant_tokens_get_by_code($code);
	...
	if (($ok) && (! api_oauth_onedotfive_grant_tokens_is_timely($grant_token))){
	...
	api_oauth_onedotfive_grant_tokens_delete($grant_token);

	# Generate the access token (check to make sure one doesn't already exist)

	$access_token = api_oauth_onedotfive_access_tokens_get_for_user_and_key($user, $key_row);
	...
	if (! $access_token){
		...
		$rsp = api_oauth_onedotfive_access_tokens_create($key_row, $user, $perms, $ttl);
