<?php
	...
	$token_row = api_oauth_onedotfive_access_tokens_get_for_user_and_key($GLOBALS['cfg']['user'], $key_row);
	...
	if (post_isset('delete') && crumb_check($crumb_key)){
		...
		if ($conf){
			$rsp = api_oauth_onedotfive_access_tokens_delete($token_row);
			$GLOBALS['smarty']->assign_by_ref("delete_rsp", $rsp);
		}		

		$GLOBALS['smarty']->assign_by_ref("token", $token_row);

		$GLOBALS['smarty']->display("page_api_oauth_onedotfive_token_delete.txt");
		exit();
	}
		...
		if (! api_oauth_onedotfive_access_tokens_is_valid_permission($perms)){
			...
			$update_rsp = api_oauth_onedotfive_access_tokens_update($token_row, $update);
	...
	$GLOBALS['smarty']->display("page_api_oauth_onedotfive_token.txt");
	exit();
?>
