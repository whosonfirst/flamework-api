<?php
	...
	if ((post_isset("done")) && (crumb_check($crumb_key))){
		...
		if (($ok) && (! api_oauth_onedotfive_access_tokens_is_valid_permission($perms))){
		...
		if (($ok) && ($conf)){
			...
			if ($ok){
				...
				$rsp = api_oauth_onedotfive_access_tokens_create($key, $GLOBALS['cfg']['user'], $perms, $ttl);
				$token = $rsp['token'];
	...
	$GLOBALS['smarty']->display("page_api_oauth_onedotfive_authenticate_like_magic.txt");
	exit();
