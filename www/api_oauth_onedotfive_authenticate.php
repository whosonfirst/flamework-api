<?php
	...
	loadlib("api_oauth_onedotfive_grant_tokens");
	...
	$perms_map = api_oauth_onedotfive_access_tokens_permissions_map();
	$GLOBALS['smarty']->assign_by_ref("permissions", $perms_map);

	$ttl_map = api_oauth_onedotfive_access_tokens_ttl_map();
	...
	if (request_isset("self")){
		...
		if ($token_row = api_oauth_onedotfive_access_tokens_get_for_user_and_key($GLOBALS['cfg']['user'], $key_row)){
			...
			if (! api_oauth_onedotfive_access_tokens_is_valid_permission($perms)){
				$GLOBALS['smarty']->assign("error", "bad_perms");
			}

			else {
				$rsp = api_oauth_onedotfive_access_tokens_create($key_row, $GLOBALS['cfg']['user'], $perms, $ttl);
		...
		$GLOBALS['smarty']->display("page_api_oauth_onedotfive_authenticate_self.txt");
		exit();
	}
	...
	if (($ok) && (! api_oauth_onedotfive_access_tokens_is_valid_permission($scope, "string perms"))){
	...
	if (($ok) && ($token = api_oauth_onedotfive_grant_tokens_get_for_user_and_key($GLOBALS['cfg']['user'], $key_row))){

		if (api_oauth_onedotfive_grant_tokens_is_timely($token)){
			...
			api_oauth_onedotfive_grant_tokens_delete($token);
		}
	}

	# Do we already have an access token (with the same perms) for this user?

	if (($ok) && ($token_row = api_oauth_onedotfive_access_tokens_get_for_user_and_key($GLOBALS['cfg']['user'], $key_row))){

		$perms_map = api_oauth_onedotfive_access_tokens_permissions_map("string keys");
		...
		if ($perms == $token_row['perms']){
			...
			$rsp = api_oauth_onedotfive_grant_tokens_create($key_row, $GLOBALS['cfg']['user'], $perms);
	...
	if (($ok) && (post_isset('done')) && (crumb_check($crumb_key))){
		...
		if (post_str("confirm") == "YES, I AGREE"){
			...
			$rsp = api_oauth_onedotfive_grant_tokens_create($key_row, $GLOBALS['cfg']['user'], $perms, $ttl);
	...
	$GLOBALS['smarty']->display("page_api_oauth_onedotfive_authenticate.txt");
	exit();
